<?php
  ...
  $today = date('Y-m-d');
  $sql_result = null;
  $sql_data = $conndb->query("SELECT * FROM `member` WHERE `group` = 'fighter' ORDER BY `id` DESC");
  $sql_data->execute();
  $sql_result = $sql_data->fetchAll();
                ...
                <table id="example1" class="table table-sm table-hover" style="color: black;">
                  ...
                  <tbody>
                    <?php foreach ($sql_result as $row) : ?>
                      <tr>
                        <td> <a href="fighter_profile.php?id=<?= $row['id']; ?>"><img src=" <?= 'http://172.16.0.3/fighterimg/img/' . $row['image'] ?>"></a></td>
                        <td> <?= $row['fname'] ?></td>
